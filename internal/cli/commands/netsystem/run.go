...
import (
	"context"
	"dotfiles/internal/cli/utils/glossutils"
	"dotfiles/internal/cli/utils/httputils"
	"sync"
	"sync/atomic"
...
func Run(ctx context.Context, _ *cli.Command) error {
	...
	tableData := glossutils.NewMappingData(pingCols...)
	...
	r := Service{httpSupplier: httputils.New(), model: &m}

	return r.Run(ctx)
}

type Service struct {
	httpSupplier *httputils.Supplier
	model        *model
}
